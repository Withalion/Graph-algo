...
public class CalculateController {
    ...
    public List<Integer> calculate(int groupSize){
        ...
        /*
        List<List<Integer>> generatorsList = createGenerators();


        //start from generator list size 1 and increase after checking all
        int generatorsListSize = 1;
        int generatorsDoubleVariability = 1;
        boolean generatorsFound = false;
        while(!generatorsFound) {
            //if first iteration check if n is odd
            if (generatorsListSize == 1) {
                if (userNumber % 2 == 1) {
                    generatorsListSize++;
                } else {
                    for (int element = 1; element < userNumber; element++) {
                        if (!isGenerated(0, generatorsList.get(generatorsList.size()-1).get(0), 1, element)) break;
                        else generatorsFound = true;
                    }
                }
            }
            //in other iterations just find out if every element can be generated by some generators
            else {
                List<Integer> currentGeneratorList = new ArrayList<>();
                int currentDoubleVariability = 1;
                while (currentGeneratorList.size() < generatorsListSize){
                    for (List<Integer> generators: generatorsList){
                        if ((generatorsListSize - currentGeneratorList.size()) % 2 == 1 && generators.size() == 1
                        && !currentGeneratorList.contains(generators.get(0))){
                                currentGeneratorList.addAll(generators);
                                break;
                        }
                        else if ((generatorsListSize - currentGeneratorList.size()) % 2 == 0 && generators.size() == 2
                                && !currentGeneratorList.contains(generators.get(0))){
                            if (currentDoubleVariability < generatorsDoubleVariability){
                                currentDoubleVariability++;
                            }
                            else {
                                currentGeneratorList.addAll(generators);
                                generatorsDoubleVariability++;
                                break;
                            }
                        }
                    }
                ...
                int generableElements = 0;
                for (int element = 1; element < userNumber; element++) {
                    if (isGeneratedByMultiple(currentGeneratorList, element)){
                        generableElements++;
                    }
                    else {
                        break;
                ...
                if (generableElements == userNumber - 1) {
                    graphDegree = currentGeneratorList.size();
                    System.out.println("The graph degree is " + graphDegree);
                    break;
                }
                else if (generatorsDoubleVariability > generatorsList.size() * 2){
                    generatorsDoubleVariability = 1;
                    generatorsListSize++;
                }
                else generatorsDoubleVariability++;
            }
        }*/
    }
    ...
    /*
    private boolean isGenerated(int beginValue, int generator, int jump, int endValue){
        if ((beginValue + generator) % groupSize == endValue) return true;
        else if (jump < graphMean) {
            return isGenerated(beginValue + generator, generator, 2, endValue);
        }
        else return false;
    }

    private boolean isGeneratedByMultiple(List<Integer> generators, int endValue){
        for (Integer generator : generators){
            if ((generator) % groupSize == endValue) return true;
            else {
                for (Integer generator2 : generators) {
                    if (isGenerated(generator, generator2, 2, endValue)) {
                        return true;
                    }
                }
            }
    ...
    //creates generator pairs or self-inverse generator
    private List<List<Integer>> createGenerators(){
        List<List<Integer>> generatorsList = new ArrayList<>();
        for (int i = 1; i <= groupSize / 2; i++) {
            List<Integer> generatorsVector = new ArrayList<>();
            if (i != groupSize / 2) {
                generatorsVector.add(i);
                generatorsVector.add(groupSize - i);
            }
            else generatorsVector.add(i);
            generatorsList.add(generatorsVector);
        }
